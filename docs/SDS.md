...
1. **Complete allocation detection**: Before the cost center loop, the processor checks whether `yoy_allocated` contains any key that matches a known cost center name. If it does, `yoy_allocated` is treated as a complete allocation (CE populated all cost centers that had costs for that period). In this case, cost centers absent from `yoy_allocated` receive a YoY cost of `$0` — their costs are assumed to be already rolled up into another cost center's allocated total, so falling back to the workload sum would double-count them. If no matching key is found (e.g., `yoy_allocated` contains only sentinel keys like `"No cost category"` because the period predates the Cost Category definition), all cost centers fall back to summing their YoY workload costs.

2. **Per-cost-center lookup**: For each cost center in the loop, if the cost center name is present as a key in that period's allocated dict, the processor uses the allocated value; otherwise it applies the fallback determined in step 1.
